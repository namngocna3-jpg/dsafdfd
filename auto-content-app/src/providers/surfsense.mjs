...
// 2 che do:
//   - SURFSENSE_MODE=make  : app POST sang Make webhook (kem danh sach kinds), Make lo sinh + giao Drive. (mac dinh)
//   - SURFSENSE_MODE=rest  : app goi thang REST SurfSense (upload -> trigger -> poll).
...
// Che do make: Make scenario lo het -> coi nhu ho tro MOI loai.
// Che do rest: SurfSense sinh video/podcast/report/anh; mindmap khong chac -> mac dinh khong.
const REST_SUPPORTS = new Set(["video", "audio", "pptx", "pdf", "image"]);
...
export default {
  ...
  supports(kind) {
    return MODE === "make" ? true : REST_SUPPORTS.has(kind);
  },
...
async function viaMake({ driveFileId, fileUrl, title, kinds }) {
  const url = process.env.MAKE_WEBHOOK_URL;
  if (!url) throw new Error("Thieu MAKE_WEBHOOK_URL");
  const res = await fetch(url, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ driveFileId, fileUrl, title, kinds, source: "auto-content-app" }),
  });
  if (!res.ok) throw new Error(`Make webhook loi HTTP ${res.status}: ${await res.text()}`);
  // Make lo tat ca kinds -> danh dau tung kind da ban giao
  const outputs = {};
  for (const k of kinds) outputs[k] = "make";
  return { ok: true, outputs, note: "Da ban sang Make; Make se sinh + luu Drive." };
}

// --- Che do REST: goi thang SurfSense ---
async function viaRest({ filePath, title, outDir, kinds }) {
  ...
  // 3) KICH SINH: SurfSense sinh qua agent/Celery, khong phai 1 REST co dinh.
  //    Dien path that lay tu /docs cua instance vao SS_GENERATE_PATH (vd: /chats/.../messages).
  ...
  const gen = await fetch(`${API}${genPath}`, {
    ...
    body:
      process.env.SS_GENERATE_BODY ||
      JSON.stringify({ message: `Generate [${kinds.join(", ")}] from document: ${title}` }),
  });
  if (!gen.ok) throw new Error(`SurfSense trigger loi ${gen.status}`);

  // 4) poll video-presentations toi khi ready (best-effort cho kind video)
  for (let i = 0; i < 60; i++) {
    ...
    if (r.ok) {
      ...
      if (latest && /ready|completed/i.test(latest.status || "")) {
        const outputs = {};
        for (const k of kinds) outputs[k] = { videoPresentationId: latest.id };
        return { ok: true, outputs, note: "SurfSense rest: best-effort theo video-presentation gan nhat." };
      }
    }
    await new Promise((res) => setTimeout(res, 10_000));
  }
  throw new Error("SurfSense: het luot poll, chua ready");
}
